...
class ppomnivaAjaxModuleFrontController extends ModuleFrontController
{
    ...
    private function fetchNominatim(string $url): ?array
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'User-Agent: PrestaShop-PPOmniva/' . (defined('_PS_VERSION_') ? _PS_VERSION_ : '0') . ' (+https://prestapro.lt)',
            ],
        ]);
    ...
    {
        // Hardcoded for Omniva markets so we don't pay for a Country DB
     ...
     * Creates or updates the ppomniva_order row.
     */
    private function saveTerminalSelection(): void
    {
        ...
        $db = Db::getInstance();
        $table = bqSQL('ppomniva_order');
        $idCart = (int) $cart->id;
        $now = date('Y-m-d H:i:s');

        // Check if row already exists for this cart
        $existing = $db->getValue(
            'SELECT `id_ppomniva_order` FROM `' . _DB_PREFIX_ . $table . '`
        ...
        if ($existing) {
            $db->update($table, [
                ...
                'date_upd' => pSQL($now),
            ], '`id_ppomniva_order` = ' . (int) $existing);
        } else {
            $db->insert($table, [
                'id_cart' => $idCart,
                'terminal_id' => $terminalId,
