/**
 * PPOmniva Checkout — Pickup terminal selector & courier extra fields.
...
(function () {
    ...
    function loadLeaflet(callback) {
        ...
        js.onerror = function () {
            leafletLoading = false;
            console.error('PPOmniva: Failed to load Leaflet.');
        };
    ...
    function renderTerminalList(container, terminals) {
        var listEl = container.querySelector('.ppomniva-pickup__list-items');
        if (!listEl) return;

        if (!terminals.length) {
            listEl.innerHTML = '<div class="ppomniva-pickup__empty">' + escapeHtml(tr('No terminals found.')) + '</div>';
            return;
        }

        var html = '';
        terminals.forEach(function (t) {
            // Omniva's API exposes only two terminal_type values for the
            ...
            if (tType === 3) {
                typeLabel = tr('Locker');
                typeCls = 'ppomniva-pickup__item-type--locker';
            } else {
                typeLabel = tr('Shop');
                typeCls = 'ppomniva-pickup__item-type--shop';
            }
            ...
            var codBadge = (parseInt(t.cod_enabled, 10) === 1)
                ? '<span class="ppomniva-pickup__item-cod ppomniva-pickup__item-cod--yes" title="' + escapeHtml(tr('Cash on Delivery available')) + '">€</span>'
                : '<span class="ppomniva-pickup__item-cod ppomniva-pickup__item-cod--no" title="' + escapeHtml(tr('No Cash on Delivery')) + '">€</span>';

            var distText = formatDistance(t._distanceKm);
            var distHtml = distText
                ? '<span class="ppomniva-pickup__item-distance">' + escapeHtml(distText) + '</span>'
                : '';

            html += '<div class="ppomniva-pickup__item" data-terminal-id="' + escapeHtml(String(t.terminal_id)) + '">'
                + '<div class="ppomniva-pickup__item-icon">'
                + '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">'
                + '<path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 1 1 18 0z"/>'
                + '<circle cx="12" cy="10" r="3"/></svg>'
                + '</div>'
                + '<div class="ppomniva-pickup__item-info">'
                + '<span class="ppomniva-pickup__item-name">' + escapeHtml(t.display_name || t.name) + '</span>'
                + '<span class="ppomniva-pickup__item-address">' + escapeHtml(t.address) + ', ' + escapeHtml(t.city) + ' ' + escapeHtml(t.zip) + '</span>'
                + '</div>'
                + distHtml
                + codBadge
                + '<span class="ppomniva-pickup__item-type ' + typeCls + '">' + escapeHtml(typeLabel) + '</span>'
                + '</div>';
        });
    ...
    function addMarkers(terminals) {
        ...
        terminals.forEach(function (t) {
            ...
            var popupContent = '<strong>' + escapeHtml(t.display_name || t.name) + '</strong>'
                ...
                + '<br><br><button type="button" class="ppomniva-pickup__map-select-btn" data-terminal-id="'
    ...
    function refreshMapView() {
        ...
        var mapEl = document.getElementById('ppomniva-map');
    ...
    function getSelectorElements(container) {
        return [
            container.querySelector('.ppomniva-pickup__toolbar'),
            container.querySelector('.js-ppomniva-map-wrap'),
            container.querySelector('.js-ppomniva-list-wrap'),
        ];
    }

    function showSelectedState(container, terminal) {
        var selectedEl = container.querySelector('.js-ppomniva-selected');
        ...
        var hiddenInput = container.querySelector('.js-ppomniva-terminal-id');
        if (hiddenInput) hiddenInput.value = terminal.terminal_id;

        if (selectedEl) {
            var nameEl = selectedEl.querySelector('.js-ppomniva-selected-name');
            var addrEl = selectedEl.querySelector('.js-ppomniva-selected-address');
        ...
        // Change later.
        var nearestStatus = container.querySelector('.js-ppomniva-nearest-status');
        if (nearestStatus) {
    ...
    function showError(container, message) {
        var errEl = container.querySelector('.ppomniva-pickup__error');
        if (errEl) {
    ...
    function showLoading(container) {
        var listEl = container.querySelector('.ppomniva-pickup__list-items');
        if (listEl) {
            listEl.innerHTML = '<div class="ppomniva-pickup__loading">' + escapeHtml(tr('Loading terminals…')) + '</div>';
        }
    }
    ...
    function initPickupSelector(container) {
        ...
        if (!ajaxUrl) {
            console.error('PPOmniva: data-ajax-url missing on pickup container.');
            return;
        }
        ...
        loadTerminals(ajaxUrl, country, function (err, terminals) {
            ...
            loadLeaflet(function () {
                initMap('ppomniva-map', terminals);
                ...
                if (mapEl) {
                    mapEl.addEventListener('click', function (e) {
                        var btn = e.target.closest('.ppomniva-pickup__map-select-btn');
                        if (!btn) return;
        ...
        // flow). `.js-ppomniva-search` is kept here only to remain
        // backwards-compatible with the old class name in case a custom
        // theme still references it.
        var searchInputs = [
            container.querySelector('.js-ppomniva-list-search'),
            container.querySelector('.js-ppomniva-search'),
        ].filter(Boolean);
        ...
        var nearestInput = container.querySelector('.js-ppomniva-nearest-input');
        var nearestBtn = container.querySelector('.js-ppomniva-nearest-btn');
        var nearestStatus = container.querySelector('.js-ppomniva-nearest-status');
        ...
        if (listEl) {
            listEl.addEventListener('click', function (e) {
                var item = e.target.closest('.ppomniva-pickup__item');
                if (!item) return;
        ...
        var changeBtn = container.querySelector('.ppomniva-pickup__change-btn');
        if (changeBtn) {
    ...
    function initCourierFields(container) {
        ...
        if (!ajaxUrl) {
            console.error('PPOmniva: data-ajax-url missing on courier container.');
            return;
        }
        ...
        container.addEventListener('change', function (e) {
            if (e.target.closest('.js-ppomniva-extra-field')) {
                debouncedSave();
            }
        });

        container.addEventListener('input', function (e) {
            var el = e.target.closest('.js-ppomniva-extra-field');
    ...
    function saveCourierFields(ajaxUrl, container) {
        var fields = container.querySelectorAll('.js-ppomniva-extra-field');
        var data = {};
        ...
        ajax('POST', url, params, function (status, resp) {
            if (status >= 200 && status < 300 && resp && resp.success) {
                ...
                console.warn('PPOmniva: Failed to save extra fields.', resp);
            }
        });
    }
    ...
    function initAll() {
        var pickupContainer = document.querySelector('.ppomniva-pickup');
        if (pickupContainer) {
            initPickupSelector(pickupContainer);
            ensureExtraContentVisible(pickupContainer);
        }

        var courierContainer = document.querySelector('.ppomniva-courier');
        if (courierContainer) {
