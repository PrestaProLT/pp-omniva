...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        console.error('PPOmniva: Failed to parse URLs', e);
        return;
    }
    ...
    var terminalSearchInput = document.getElementById('ppomniva-terminal-search');
    ...
    var terminalResults = document.getElementById('ppomniva-terminal-results');
    if (terminalResults) {
    ...
    function toggleTerminalPicker() {
        var picker = document.getElementById('ppomniva-terminal-picker');
        if (!picker) return;
    ...
    function selectTerminal(t) {
        var hidden = document.getElementById('ppomniva-terminal-id');
        if (hidden) hidden.value = t.terminal_id;

        var current = document.getElementById('ppomniva-terminal-current');
        if (current) {
            var sub = (t.address || '') + (t.city ? ', ' + t.city : '');
    ...
    function saveShipment(btn) {
        var weightInput = document.getElementById('ppomniva-weight-input');
        var packagesInput = document.getElementById('ppomniva-packages-input');
        var codInput = document.getElementById('ppomniva-cod-input');
        ...
        var terminalIdInput = document.getElementById('ppomniva-terminal-id');
        ...
        fetch(urls.update_shipment, {
            ...
            .then(function (data) {
                ...
                if (data.success) {
                    ...
                    var codDisplay = document.getElementById('ppomniva-cod-amount');
    ...
    function printManifest(btn) {
        ...
        fetchAndPrint(
            ...
            'omniva-manifest-' + manifestId + '.pdf'
        );
    }
    ...
    function renderTrackingDetails(tracking) {
        var container = document.getElementById('ppomniva-tracking-details');
        container.style.display = 'block';

        var html = '<div class="ppomniva-panel__section"><strong>Tracking History</strong>';
        ...
        for (var packNo in tracking) {
            ...
            html += '<div class="ppomniva-tracking-pack"><small><strong>' + escapeHtml(packNo) + '</strong>';
            ...
            if (events.length === 0) {
                html += '<div class="ppomniva-tracking-row">No tracking data yet</div>';
            } else {
                for (var i = 0; i < events.length; i++) {
                    var ev = events[i];
                    var location = (ev.location && (ev.location.place || ev.location.city)) || '';
                    html += '<div class="ppomniva-tracking-row">'
                        + '<span class="ppomniva-tracking-date">' + escapeHtml(ev.date || '') + '</span> '
                        + '<span class="ppomniva-tracking-status">' + escapeHtml(ev.pack_status_text || ev.event || '') + '</span>'
                        + (location ? ' <span class="ppomniva-tracking-terminal">(' + escapeHtml(location) + ')</span>' : '')
                        + '</div>';
                }
            }
            html += '</div>';
    ...
    function setLoading(btn, loading) {
        if (loading) {
            ...
            btn.innerHTML = '<i class="material-icons ppomniva-spin">sync</i> Loading...';
        } else {
            btn.disabled = false;
    ...
    function showNotice(type, message) {
        ...
        var existing = panel.querySelectorAll('.ppomniva-notice');
        existing.forEach(function (el) { el.remove(); });

        var text = String(message || '');
        var notice = document.createElement('div');
        notice.className = 'ppomniva-notice ppomniva-notice--' + type;
