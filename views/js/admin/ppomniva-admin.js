/*
 * Lightweight enhancements shared across all PPOmniva admin pages.
 * - Loading state on async submit buttons (forms with [data-ppo-loading])
...
(function () {
    ...
    // Apply ppo-loading state to a button/element. Idempotent.
    function setLoading(el, isLoading) {
        ...
        if (isLoading) {
            ...
            el.classList.add('ppo-loading');
            el.setAttribute('disabled', 'disabled');
        } else {
            el.classList.remove('ppo-loading');
    ...
    // Mark forms that opt-in via data-ppo-loading; show spinner on submit.
    document.querySelectorAll('form[data-ppo-loading]').forEach(function (form) {
    ...
    window.PPOmnivaAdmin = window.PPOmnivaAdmin || {};
    window.PPOmnivaAdmin.setLoading = setLoading;

    window.PPOmnivaAdmin.showNotice = function (container, type, message) {
        if (!container) return;
        container.querySelectorAll('.ppo-notice').forEach(function (n) { n.remove(); });
        var el = document.createElement('div');
        el.className = 'ppo-notice ppo-notice--' + type;
