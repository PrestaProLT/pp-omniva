...
{
    /**
     * displayAdminOrderMainBottom hook — renders Omniva panel beneath the
    ...
    public function hookDisplayAdminOrderMainBottom(array $params): string
    {
        ...
        // Check if this is a Omniva order
        ...
        // "Generate label" and gets a Omniva code 70-74 rejection.
        ...
        return $twig->render('@Modules/ppomniva/views/templates/admin/order_panel.html.twig', [
            'omniva_order' => [
                'status' => $vo['status'],
            ...
            'id_order' => $orderId,
            'urls' => [
                'generate_label' => $router->generate('ps_ppomniva_generate_label'),
                'print_label' => $router->generate('ps_ppomniva_print_label'),
                'print_manifest' => $router->generate('ps_ppomniva_print_manifest'),
                'close_manifest' => $router->generate('ps_ppomniva_close_manifest'),
                'call_courier' => $router->generate('ps_ppomniva_call_courier'),
                'tracking' => $router->generate('ps_ppomniva_tracking'),
                'warehouses' => $router->generate('ps_ppomniva_warehouses'),
                'manifests' => $router->generate('ps_ppomniva_manifests'),
                'update_shipment' => $router->generate('ps_ppomniva_update_shipment'),
                'regenerate_label' => $router->generate('ps_ppomniva_regenerate_label'),
                'order_terminals' => $router->generate('ps_ppomniva_order_terminals'),
            ],
            'tracking_base_url' => 'https://mano.omniva.lt/track/',
        ]);
    }
    ...
    private function resolveWarehouseForOrder(int $idWarehouse, int $idShop = 0): ?array
    {
        ...
        if ($idWarehouse > 0) {
            $row = $db->getRow(
                'SELECT * FROM `' . _DB_PREFIX_ . 'ppomniva_warehouse`
                 WHERE `id_ppomniva_warehouse` = ' . $idWarehouse
            );
        }
        ...
        try {
            $router = $this->get('router');
            if ($row && !empty($row['id_ppomniva_warehouse'])) {
                $editUrl = $router->generate('ps_ppomniva_warehouse_edit', [
                    'id' => (int) $row['id_ppomniva_warehouse'],
                ]);
            } else {
                $editUrl = $router->generate('ps_ppomniva_warehouses');
            }
        } catch (\Throwable $e) {
        ...
        return [
            ...
            'exists' => true,
            'missing_fields' => \PrestaShop\Module\PPOmniva\Service\LabelGenerationService::validateWarehouseFields($row),
            'edit_url' => $editUrl,
        ];
    }
    ...
    public function hookDisplayBackOfficeHeader(): string
    {
        ...
        $isPpomnivaPage = str_starts_with($routeName, 'ps_ppomniva_');

        if (!$isOrderPage && !$isPpomnivaPage) {
            return '';
        }

        $moduleUri = $this->getPathUri();
        $moduleDir = _PS_MODULE_DIR_ . 'ppomniva/';
        ...
        if ($isPpomnivaPage) {
            $output .= '<link rel="stylesheet" href="' . $asset('views/css/admin/ppomniva-admin.css') . '">';
            $output .= '<script src="' . $asset('views/js/admin/ppomniva-admin.js') . '" defer></script>';
        }
    ...
    /**
     * actionObjectOrderUpdateAfter — keep ppomniva_order in sync when the
     * merchant changes the carrier on an existing order in admin. Without
     * this, ppomniva_order.id_carrier and terminal_info stay frozen at
    ...
    public function hookActionObjectOrderUpdateAfter(array $params): void
    {
        ...
        $db = Db::getInstance();
        $vo = $db->getRow(
            'SELECT `id_carrier`, `terminal_id` FROM `' . _DB_PREFIX_ . 'ppomniva_order`
             WHERE `id_order` = ' . (int) $order->id
        );

        if (!$vo) {
            return; // not a Omniva order
        }
        ...
        $pickupRef = (int) Configuration::get('PPOMNIVA_PICKUP_ID_REF');
        $courierRef = (int) Configuration::get('PPOMNIVA_COURIER_ID_REF');
        ...
        $db->update('ppomniva_order', $update, '`id_order` = ' . (int) $order->id);
    }
}
