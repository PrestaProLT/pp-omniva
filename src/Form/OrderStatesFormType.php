...
/**
 * Mapping form: each Omniva pack_status code → a PrestaShop OrderState id.
 *
...
class OrderStatesFormType extends TranslatorAwareType
{
    ...
    public const FIELDS = [
        ...
        'ready' => [
            'config' => 'PPOMNIVA_STATE_READY',
            'label' => 'Label generated — ready to ship',
            'help' => 'Applied right after a label is successfully generated. Use the module\'s "Omniva siunta paruošta" state — do NOT map this to a payment state.',
        ],
        'error' => [
            'config' => 'PPOMNIVA_STATE_ERROR',
            'label' => 'Label generation failed',
            'help' => 'Applied when Omniva rejects the shipment so failed orders are easy to spot. Use the module\'s "Klaida Omniva siuntoje" state — do NOT map this to a payment state.',
        ],

        // --- Tracking pack_status sync --------------------------------------
        'at_sender' => [
            'config' => 'PPOMNIVA_STATE_AT_SENDER',
        ...
        'picked_up' => [
            'config' => 'PPOMNIVA_STATE_PICKED_UP',
            'label' => 'Picked up by courier (pack_status 1)',
            'help' => 'Omniva\'s courier has collected the parcel from your warehouse.',
        ],
        'at_terminal' => [
            'config' => 'PPOMNIVA_STATE_AT_TERMINAL',
            'label' => 'At Omniva terminal (pack_status 2)',
            'help' => 'Parcel arrived at a Omniva hub and is being sorted.',
        ],
        'out_for_delivery' => [
            'config' => 'PPOMNIVA_STATE_OUT_FOR_DELIVERY',
            'label' => 'Out for delivery (pack_status 3)',
            'help' => 'Last-mile courier is heading to the recipient.',
        ],
        'at_pickup_point' => [
            'config' => 'PPOMNIVA_STATE_AT_PICKUP_POINT',
        ...
        'in_transit' => [
            'config' => 'PPOMNIVA_STATE_IN_TRANSIT',
            'label' => 'In transit (pack_status 7)',
            'help' => 'Generic in-transit signal Omniva emits between hub events.',
        ],
        'delivered' => [
            'config' => 'PPOMNIVA_STATE_DELIVERED',
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        foreach (self::FIELDS as $key => $meta) {
            $builder->add($key, ChoiceType::class, [
                'label' => $this->trans($meta['label'], 'Modules.Ppomniva.Admin'),
                'help' => $this->trans($meta['help'], 'Modules.Ppomniva.Admin'),
                'choices' => $choices,
                'required' => false,
                'placeholder' => false,
    ...
    private function buildOrderStateChoices(): array
    {
        ...
        $choices = [
            $this->trans('— Do not change order state —', 'Modules.Ppomniva.Admin') => 0,
        ];
