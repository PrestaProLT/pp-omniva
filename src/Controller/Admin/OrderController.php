...
use Configuration;
use Db;
use PrestaShop\Module\PPOmniva\Api\CourierInvitationXmlBuilder;
use PrestaShop\Module\PPOmniva\Api\OmnivaApiClient;
use PrestaShop\Module\PPOmniva\Service\ApiLogger;
use PrestaShop\Module\PPOmniva\Service\LabelGenerationService;
...
class OrderController extends FrameworkBundleAdminController
{
    ...
    public function printLabel(Request $request): Response
    {
        ...
        try {
            ...
            $apiClient = (new OmnivaApiClient())->setShopId($shopId ?: null);
            $format = Configuration::get('PPOMNIVA_LABEL_FORMAT', null, null, $shopId ?: null) ?: 'a4';
            ...
            return new Response($pdf, 200, [
                ...
                'Content-Disposition' => 'inline; filename="omniva-label-' . $orderId . '.pdf"',
    ...
    public function printManifest(Request $request): Response
    {
        ...
        try {
            ...
            return new Response($pdf, 200, [
                'Content-Type' => 'application/pdf',
                'Content-Disposition' => 'inline; filename="omniva-manifest-' . $manifestId . '.pdf"',
                'Content-Length' => strlen($pdf),
            ]);
        } catch (\Throwable $e) {
    ...
    public function callCourier(Request $request): JsonResponse
    {
        ...
        // Same-day cutoff: Omniva rejects same-day pickup requests submitted after 15:00.
        ...
        try {
            ...
            $manifest = Db::getInstance()->getRow(
                'SELECT * FROM `' . _DB_PREFIX_ . 'ppomniva_manifest`
            ...
            Db::getInstance()->update('ppomniva_manifest', [
    ...
    public function tracking(Request $request): JsonResponse
    {
        ...
        try {
            $apiClient = (new OmnivaApiClient())->setShopId($this->getOrderShopId($orderId) ?: null);
            $allTracking = [];
            ...
            ApiLogger::logException(
                ...
                $e,
                'tracking.omniva.com/api/v1/events',
                $orderId
            );
    ...
    /**
     * Get Omniva order data for the admin sidebar panel.
     */
    #[AdminSecurity("is_granted('read', request.get('_legacy_controller'))")]
    public function orderData(Request $request): JsonResponse
    {
        ...
        $vo = Db::getInstance()->getRow(
            'SELECT * FROM `' . _DB_PREFIX_ . 'ppomniva_order`
             WHERE `id_order` = ' . (int) $orderId
        );

        if (!$vo) {
            return new JsonResponse(['success' => false, 'message' => 'Not a Omniva order.'], 404);
        }
     ...
     * Omniva order. Only allowed before the label is generated — once Omniva
    ...
    public function updateShipmentDetails(Request $request): JsonResponse
    {
        ...
        $db = Db::getInstance();
        $vo = $db->getRow(
            'SELECT `status`, `id_carrier` FROM `' . _DB_PREFIX_ . 'ppomniva_order`
        ...
        if ($request->request->has('terminal_id')) {
            $pickupRef = (int) Configuration::get('PPOMNIVA_PICKUP_ID_REF');
            ...
            $terminal = $db->getRow(
                'SELECT * FROM `' . _DB_PREFIX_ . 'ppomniva_terminal`
        ...
        $db->update('ppomniva_order', $update, '`id_order` = ' . $orderId);
    ...
    public function terminals(Request $request): JsonResponse
    {
        ...
        $rows = $db->executeS(
            'SELECT `terminal_id`, `display_name`, `name`, `address`, `city`, `zip`,
             ...
             FROM `' . _DB_PREFIX_ . 'ppomniva_terminal`
    ...
    /**
     * Reset a Omniva order so a fresh label can be generated for it.
    ...
    public function regenerateLabel(Request $request): JsonResponse
    {
        ...
        if (!$hasLabel) {
            return new JsonResponse([
                'success' => false,
                'message' => 'This order has no existing label — use "Send to Omniva" to generate the first one.',
            ], 409);
        }
        ...
        return new JsonResponse([
            'success' => true,
            'message' => 'Label reset. Click "Send to Omniva" to generate a new label.',
            'attempt_count' => (int) ($vo['attempt_count'] ?? 1) + 1,
        ]);
    }
    ...
    /**
     * Shop id of an order, used to bind the Omniva API client to that shop's
    ...
    private function updateLocalStatus(int $orderId, array $allTracking): void
    {
        ...
        // entry in PPOMNIVA_STATE_* is either 0 ("don't change") or a
    ...
    /**
     * Translate a Omniva pack_status into a PrestaShop order-state id by
     ...
     * Omniva emits both as "parcel arrived at locker / shop, awaiting
    ...
    private function resolvePsStateForPackStatus(int $packStatus, string $event, bool $isFinal, ?int $idShop = null): int
    {
        ...
        switch ($packStatus) {
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_AT_SENDER:
                return (int) Configuration::get('PPOMNIVA_STATE_AT_SENDER', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_PICKED_UP:
                return (int) Configuration::get('PPOMNIVA_STATE_PICKED_UP', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_AT_TERMINAL:
                return (int) Configuration::get('PPOMNIVA_STATE_AT_TERMINAL', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_OUT_FOR_DELIVERY:
                return (int) Configuration::get('PPOMNIVA_STATE_OUT_FOR_DELIVERY', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_AT_PICKUP_POINT_5:
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_AT_PICKUP_POINT_6:
                return (int) Configuration::get('PPOMNIVA_STATE_AT_PICKUP_POINT', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_IN_TRANSIT:
                return (int) Configuration::get('PPOMNIVA_STATE_IN_TRANSIT', null, null, $idShop);
            case \PrestaShop\Module\PPOmniva\Api\OmnivaApiClient::PACK_STATUS_DELIVERED:
                // Only honour the "delivered" mapping when Omniva's event
                ...
                return $isFinal ? (int) Configuration::get('PPOMNIVA_STATE_DELIVERED', null, null, $idShop) : 0;
            default:
                return 0;
        }
    }
        ...
        if ($warehouseId > 0) {
            $warehouse = $db->getRow(
                'SELECT * FROM `' . _DB_PREFIX_ . 'ppomniva_warehouse`
                 WHERE `id_ppomniva_warehouse` = ' . $warehouseId
            );
        }
