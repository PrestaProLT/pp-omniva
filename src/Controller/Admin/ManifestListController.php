...
class ManifestListController extends FrameworkBundleAdminController
{
    ...
    public function index(): Response
    {
        ...
        $whTable = _DB_PREFIX_ . 'ppomniva_warehouse';
        ...
        return $this->render('@Modules/ppomniva/views/templates/admin/manifests.html.twig', [
    ...
    private function loadManifests(string $whereClause, int $limit): array
    {
        ...
        return $db->executeS(
            'SELECT m.*,
             ...
             LEFT JOIN `' . $whTable . '` w ON w.`id_ppomniva_warehouse` = m.`id_warehouse`
             WHERE ' . $whereClause . '
             ORDER BY m.`id_ppomniva_manifest` DESC
             LIMIT ' . (int) $limit
        ) ?: [];
    }
