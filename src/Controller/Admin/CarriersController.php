...
class CarriersController extends FrameworkBundleAdminController
{
    ...
    public function index(
        Request $request,
        #[Autowire(service: 'prestashop.module.ppomniva.form.carrier_handler')]
        ...
        if ($request->isMethod('POST') && $form->isSubmitted() && $form->isValid()) {
            ...
            if (empty($errors)) {
                $this->addFlash(
                    'success',
                    $this->trans('Carrier settings updated.', 'Modules.Ppomniva.Admin')
                );
            } else {
                $this->flashErrors($errors);
            }

            return $this->redirectToRoute('ps_ppomniva_carriers');
        }

        return $this->render('@Modules/ppomniva/views/templates/admin/carriers.html.twig', [
    ...
    {
        return [
            'globally_enabled' => (bool) Configuration::get('PPOMNIVA_ENABLED'),
            'carriers' => [
                'courier' => $this->loadCarrierStatusFor(
                    'Omniva Courier',
                    (int) Configuration::get('PPOMNIVA_COURIER_ID_REF')
                ),
                'pickup' => $this->loadCarrierStatusFor(
                    'Omniva Pickup Point',
                    (int) Configuration::get('PPOMNIVA_PICKUP_ID_REF')
                ),
            ],
        ];
    }
