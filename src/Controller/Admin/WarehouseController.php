...
{
    private const TABLE = 'ppomniva_warehouse';
    ...
    public function index(): Response
    {
        ...
        return $this->render('@Modules/ppomniva/views/templates/admin/warehouses.html.twig', [
            'warehouses' => $rows,
            'index_url' => $this->generateUrl('ps_ppomniva_warehouses'),
            'create_url' => $this->generateUrl('ps_ppomniva_warehouse_create'),
    ...
    public function edit(Request $request, int $id): Response
    {
        $warehouse = Db::getInstance()->getRow(
            'SELECT * FROM `' . _DB_PREFIX_ . self::TABLE . '` WHERE `id_ppomniva_warehouse` = ' . $id
        );

        if (!$warehouse) {
            $this->addFlash('error', $this->trans('Warehouse not found.', 'Modules.Ppomniva.Admin'));

            return $this->redirectToRoute('ps_ppomniva_warehouses');
        }
    ...
    public function delete(int $id): RedirectResponse
    {
        $db = Db::getInstance();
        $row = $db->getRow(
            'SELECT `id_ppomniva_warehouse`, `is_default` FROM `' . _DB_PREFIX_ . self::TABLE . '`
             WHERE `id_ppomniva_warehouse` = ' . $id
        );
        ...
        if ((int) $row['is_default']) {
            $this->addFlash(
                'error',
                $this->trans('Cannot delete the default warehouse. Set another warehouse as default first.', 'Modules.Ppomniva.Admin')
            );
        ...
        $db->execute(
            'DELETE FROM `' . _DB_PREFIX_ . self::TABLE . '` WHERE `id_ppomniva_warehouse` = ' . $id
        );

        $this->addFlash('success', $this->trans('Warehouse deleted.', 'Modules.Ppomniva.Admin'));
    ...
    public function setDefault(int $id): RedirectResponse
    {
        ...
        $exists = $db->getValue(
            'SELECT COUNT(*) FROM `' . _DB_PREFIX_ . self::TABLE . '` WHERE `id_ppomniva_warehouse` = ' . $id
        );
        ...
        $db->execute(
            'UPDATE `' . _DB_PREFIX_ . self::TABLE . '`
             SET `is_default` = (CASE WHEN `id_ppomniva_warehouse` = ' . $id . ' THEN 1 ELSE 0 END)
             WHERE `id_shop` = ' . $idShop
        );

        $this->addFlash('success', $this->trans('Default warehouse updated.', 'Modules.Ppomniva.Admin'));
    ...
    private function renderWarehouseForm(Request $request, ?array $warehouse): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($data['name'] === '') {
                $errors[] = $this->trans('Name is required.', 'Modules.Ppomniva.Admin');
            }
            if ($data['city'] === '') {
                $errors[] = $this->trans('City is required.', 'Modules.Ppomniva.Admin');
            }
            if ($data['address'] === '') {
                $errors[] = $this->trans('Address is required.', 'Modules.Ppomniva.Admin');
            }
            if ($data['zip_code'] === '') {
                $errors[] = $this->trans('Postal code is required.', 'Modules.Ppomniva.Admin');
            }
            if (!in_array($data['country_code'], self::COUNTRIES, true)) {
                $errors[] = $this->trans('Country must be one of LT, LV, EE, PL.', 'Modules.Ppomniva.Admin');
            }
            ...
            if (empty($errors)) {
                ...
                if ($isEdit) {
                    $db->update(self::TABLE, $payload, '`id_ppomniva_warehouse` = ' . (int) $warehouse['id_ppomniva_warehouse']);
                    $this->addFlash('success', $this->trans('Warehouse updated.', 'Modules.Ppomniva.Admin'));
                } else {
                    $db->insert(self::TABLE, $payload);
                    $this->addFlash('success', $this->trans('Warehouse created.', 'Modules.Ppomniva.Admin'));
                }
        ...
        return $this->render('@Modules/ppomniva/views/templates/admin/warehouse_form.html.twig', [
            'is_edit' => $isEdit,
            'data' => $data,
            'errors' => $errors,
            'countries' => self::COUNTRIES,
            'cancel_url' => $this->generateUrl('ps_ppomniva_warehouses'),
            'multistore_blocked' => false,
        ]);
    }
