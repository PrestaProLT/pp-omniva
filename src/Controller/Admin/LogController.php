...
class LogController extends FrameworkBundleAdminController
{
    ...
    private const TABLE = 'ppomniva_log';
    ...
    public function index(Request $request): Response
    {
        ...
        $rows = $db->executeS(
            'SELECT `id_ppomniva_log`, `date_add`, `level`, `source`, `endpoint`,
                    `error_code`, `message`, `context`,
                    `request_body`, `response_body`, `id_order`
             FROM `' . $table . '` '
            . $where
            . ' ORDER BY `id_ppomniva_log` DESC LIMIT ' . self::PAGE_SIZE . ' OFFSET ' . (int) $offset
        ) ?: [];

        // Extract Omniva's verbatim error texts from the structured context
        ...
        foreach ($rows as &$row) {
            $row['omniva_raw'] = $this->extractRawErrors((string) ($row['context'] ?? ''));
        }
        unset($row);
        ...
        return $this->render('@Modules/ppomniva/views/templates/admin/log.html.twig', [
            'logs' => $rows,
            'total' => $total,
            'page' => $page,
            ...
            'detail_url' => $this->generateUrl('ps_ppomniva_log_detail'),
            'cleanup_url' => $this->generateUrl('ps_ppomniva_log_cleanup'),
            'index_url' => $this->generateUrl('ps_ppomniva_log'),
        ]);
    }
    ...
    public function detail(Request $request): JsonResponse
    {
        ...
        $row = Db::getInstance()->getRow(
            'SELECT * FROM `' . _DB_PREFIX_ . self::TABLE . '` WHERE `id_ppomniva_log` = ' . $id
        );
    ...
    public function cleanup(Request $request): RedirectResponse
    {
        ...
        if ($retention > 0) {
            ...
            $this->addFlash(
                'success',
                $this->trans('Log table cleared.', 'Modules.Ppomniva.Admin')
            );
        }

        return $this->redirectToRoute('ps_ppomniva_log');
    }
    ...
    /**
     * Pluck the verbatim Omniva error text(s) out of the stored context
     * JSON. ApiLogger persists `errors` as an array of structured entries
     * (`code`, `message`, `pack`, `shipment`, ...) — the `message` field is
     * exactly what Omniva's response XML contained. Returns one string
