...
class OrderStatesController extends FrameworkBundleAdminController
{
    ...
    public function index(
        Request $request,
        #[Autowire(service: 'prestashop.module.ppomniva.form.order_states_handler')]
        ...
        if ($request->isMethod('POST') && $form->isSubmitted() && $form->isValid()) {
            ...
            if (empty($errors)) {
                $this->addFlash('success', $this->trans('Order-state mapping saved.', 'Modules.Ppomniva.Admin'));
            } else {
                $this->flashErrors($errors);
            }

            return $this->redirectToRoute('ps_ppomniva_order_states');
        }

        return $this->render('@Modules/ppomniva/views/templates/admin/order_states.html.twig', [
    ...
    {
        $token = (string) Configuration::get('PPOMNIVA_CRON_TOKEN');
        if ($token === '') {
            $token = bin2hex(random_bytes(16));
            Configuration::updateValue('PPOMNIVA_CRON_TOKEN', $token);
        }

        $link = Context::getContext()->link;
        $url = $link->getModuleLink('ppomniva', 'cron', [
            'action' => 'tracking',
            'token' => $token,
        ], true);
