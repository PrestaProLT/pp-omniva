...
/**
 * Maps numeric Omniva error codes to plain-English messages aimed at a
 * non-technical merchant. Each entry has a title (what's wrong) and a fix
 * (what to do about it). Error codes and the raw Omniva text are kept on
...
class OmnivaErrorMapper
{
    ...
    private const CODES = [
        ...
        1 => [
            ...
            'title' => 'Your Omniva account has unpaid invoices.',
            'fix' => 'Contact your Omniva account manager to restore service.',
        ],
        9 => [
            'category' => self::CATEGORY_AUTH,
            'title' => 'Omniva does not recognise your username or password.',
        ...
        2 => [
            ...
            'title' => 'Omniva did not receive any data.',
        ...
        3 => [
            'category' => self::CATEGORY_REQUEST,
            'title' => 'Omniva could not read the request.',
        ...
        5 => [
            'category' => self::CATEGORY_REQUEST,
            'title' => 'The shipment data does not match what Omniva expects.',
            'fix' => 'Likely a module bug — please report this with the request body from the Logs tab.',
        ],
        8 => [
            'category' => self::CATEGORY_REQUEST,
            'title' => 'Omniva got an unexpected request type.',
        ...
        42 => [
            'category' => self::CATEGORY_PACK,
            'title' => 'A parcel\'s tracking number is already in use at Omniva.',
        ...
        45 => [
            'category' => self::CATEGORY_PACK,
            'title' => 'A tracking number is reserved by Omniva.',
            'fix' => 'Retry — the module will pick a different one.',
        ],
        46 => [
            'category' => self::CATEGORY_PACK,
            'title' => 'A parcel exceeds the 10 m³ size limit.',
            'fix' => 'Split the order into more parcels. Edit the order in the Omniva side panel — increase "Packages" so the volume is divided.',
        ],
        ...
        94 => [
            ...
            'title' => 'This delivery country combination is not supported by your Omniva contract.',
            'fix' => 'Most Omniva accounts cover Lithuania, Latvia, Estonia, Poland and Finland only. To ship beyond those, ask your Omniva manager to enable Global service for your account.',
        ],
        95 => [
            'category' => self::CATEGORY_COUNTRY,
            'title' => 'Omniva does not deliver to this country with your current service.',
            'fix' => 'For PL/FI you need the International service; for the rest of the world, the Global service. Contact your Omniva manager to enable them.',
        ],
        96 => [
        ...
        97 => [
            ...
            'fix' => 'Disable insurance for this shipment, or contact your Omniva manager.',
        ],
        98 => [
            'category' => self::CATEGORY_SERVICE,
            'title' => 'Return service is not available for this shipment.',
            'fix' => 'Disable return service in Configuration → Return service, or contact your Omniva manager to enable it on your contract.',
        ],
        115 => [
            'category' => self::CATEGORY_SERVICE,
        ...
        134 => [
            'category' => self::CATEGORY_RECEIVER,
            'title' => 'The selected pickup point / locker is no longer valid at Omniva.',
            'fix' => 'Click "Change" in the Omniva panel and re-select a pickup point, then send again. If the list looks outdated, re-sync it from the Pickup points tab first.',
        ],
    ];
    ...
    public static function describe(int $code): array
    {
        ...
        return [
            ...
            'title' => 'Omniva rejected the shipment.',
            'fix' => 'See the exact Omniva reason below. If it is not clear how to fix it, contact support with this order reference.',
        ];
    }

    /**
     * Build a plain-English summary that a non-technical merchant can act on.
     * The structured `errors` list (codes + raw Omniva text) is kept in the
    ...
    {
        if (empty($errors)) {
            return 'Omniva rejected the shipment without a specific reason. Please retry; if it keeps failing, contact support with this order reference.';
        }

        // Deduplicate: when multiple parcels in the same shipment hit the same
        // problem (e.g. all of them have the same postcode issue), Omniva
        ...
        foreach ($byCode as $error) {
            $title = $error['title'] ?: 'Omniva rejected the shipment.';
            $fix = $error['fix'];
            ...
            // Surface the exact Omniva reason inline so the merchant doesn't
            // have to open the Logs tab to see what Omniva actually said.
