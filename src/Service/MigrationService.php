...
{
    /**
     * Config key mapping: MJVP_* → PPOMNIVA_*
     */
    private const CONFIG_MAP = [
        'MJVP_API_USER' => 'PPOMNIVA_API_USER',
        'MJVP_API_PASS' => 'PPOMNIVA_API_PASS',
        'MJVP_API_LIVE_MODE' => 'PPOMNIVA_LIVE_MODE',
        'MJVP_SENDER_NAME' => 'PPOMNIVA_SENDER_NAME',
        'MJVP_SHOP_COMPANY_CODE' => 'PPOMNIVA_SENDER_COMPANY_CODE',
        'MJVP_SHOP_CONTACT' => 'PPOMNIVA_SENDER_CONTACT',
        'MJVP_SHOP_COUNTRY_CODE' => 'PPOMNIVA_SENDER_COUNTRY',
        'MJVP_SHOP_CITY' => 'PPOMNIVA_SENDER_CITY',
        'MJVP_SHOP_ADDRESS' => 'PPOMNIVA_SENDER_ADDRESS',
        'MJVP_SHOP_POSTCODE' => 'PPOMNIVA_SENDER_POSTCODE',
        'MJVP_SHOP_PHONE' => 'PPOMNIVA_SENDER_PHONE',
        'MJVP_SHOP_EMAIL' => 'PPOMNIVA_SENDER_EMAIL',
        'MJVP_COURIER_DOOR_CODE' => 'PPOMNIVA_SHOW_DOOR_CODE',
        'MJVP_COURIER_WAREHOUSE_NUMBER' => 'PPOMNIVA_SHOW_WAREHOUSE_NO',
        'MJVP_COURIER_CABINET_NUMBER' => 'PPOMNIVA_SHOW_CABINET_NO',
        'MJVP_COURIER_DELIVERY_TIME' => 'PPOMNIVA_SHOW_DELIVERY_TIME',
        'MJVP_COURIER_CALL_BEFORE_DELIVERY' => 'PPOMNIVA_SHOW_CALL_BEFORE',
        'MJVP_RETURN_SERVICE' => 'PPOMNIVA_ENABLE_RETURN_SERVICE',
        'MJVP_RETURN_DAYS' => 'PPOMNIVA_RETURN_DAYS',
        'MJVP_COURIER_DELIVERY_TIME_NWD' => 'PPOMNIVA_NWD_ENABLED',
        'MJVP_COURIER_DELIVERY_TIME_NWD10' => 'PPOMNIVA_NWD10_ENABLED',
        'MJVP_COURIER_DELIVERY_TIME_NWD12' => 'PPOMNIVA_NWD12_ENABLED',
        'MJVP_COURIER_DELIVERY_TIME_NWD8_14' => 'PPOMNIVA_NWD8_14_ENABLED',
        'MJVP_COURIER_DELIVERY_TIME_NWD14_17' => 'PPOMNIVA_NWD14_17_ENABLED',
        'MJVP_COURIER_DELIVERY_TIME_NWD18_22' => 'PPOMNIVA_NWD18_22_ENABLED',
        'MJVP_CARRIER_DISABLE_PASSPHRASE' => 'PPOMNIVA_DISABLE_PASSPHRASE',
    ];
    ...
    /**
     * Detect if mijoraomniva is installed.
     *
    ...
    public function detect(): ?array
    {
        ...
        return [
            'module_name' => 'mijoraomniva',
            'display_name' => 'Omniva (Mijora)',
            'version' => $row['version'] ?? 'unknown',
            'active' => (bool) ($row['active'] ?? false),
            'id_module' => (int) $row['id_module'],
            'matched_case' => 'mijoraomniva v' . ($row['version'] ?? '?'),
        ];
    }
    ...
    public function migrate(bool $dryRun = false): array
    {
        ...
        try {
            ...
            if (!$dryRun) {
                ...
                $log[] = 'Disabled mijoraomniva module.';
            } else {
                $log[] = '[dry-run] Would disable mijoraomniva module.';
            }
        } catch (\Throwable $e) {
    ...
    private function migrateConfig(bool $dryRun): array
    {
        ...
        foreach (self::CONFIG_MAP as $mjvpKey => $ppKey) {
            ...
            // Don't overwrite if ppomniva already has a value set
    ...
    private function migrateLabelFormat(bool $dryRun): array
    {
        ...
        $existing = Configuration::get('PPOMNIVA_LABEL_FORMAT');
        if ($existing !== false && $existing !== null && $existing !== '') {
        ...
        if (!$dryRun) {
            Configuration::updateValue('PPOMNIVA_LABEL_FORMAT', $mapped);
        }
    ...
    private function migrateCounters(bool $dryRun): array
    {
        ...
        // Pack counter — seed the shared floor (PPOMNIVA_PACK_COUNTER).
        //
        ...
        // every stored row — that is the true high-water mark Omniva has
        ...
        if ($legacyPack > 0) {
            $currentPP = (int) Configuration::getGlobalValue('PPOMNIVA_PACK_COUNTER');
            // Only adopt if legacy counter is higher (prevents going backward)
            if ($legacyPack > $currentPP) {
                if (!$dryRun) {
                    Configuration::updateGlobalValue('PPOMNIVA_PACK_COUNTER', $legacyPack);
                }
        ...
        if ($mjvpManifest !== false && $mjvpManifest !== null) {
            ...
            if (is_array($data) && isset($data['counter'])) {
                $currentPP = Configuration::getGlobalValue('PPOMNIVA_MANIFEST_COUNTER');
                $currentData = json_decode((string) $currentPP, true) ?: ['counter' => 0, 'date' => ''];

                if ((int) ($data['counter'] ?? 0) > (int) ($currentData['counter'] ?? 0)) {
                    if (!$dryRun) {
                        Configuration::updateGlobalValue('PPOMNIVA_MANIFEST_COUNTER', $mjvpManifest);
                    }
    ...
    private function migrateWarehouses(bool $dryRun): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $exists = (int) $db->getValue(
                'SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ppomniva_warehouse`
    ...
    private function migrateOrders(bool $dryRun): array
    {
        ...
        $sql = 'SELECT mo.* FROM `' . _DB_PREFIX_ . 'mjvp_orders` mo
                LEFT JOIN `' . _DB_PREFIX_ . 'ppomniva_order` po ON po.`id_order` = mo.`id_order`
                WHERE mo.`id_order` > 0
                AND po.`id_ppomniva_order` IS NULL';
    ...
    private function migrateManifests(bool $dryRun): array
    {
        ...
        $sql = 'SELECT mm.* FROM `' . _DB_PREFIX_ . 'mjvp_manifest` mm
                LEFT JOIN `' . _DB_PREFIX_ . 'ppomniva_manifest` pm ON pm.`manifest_id` = mm.`manifest_id`
                WHERE pm.`id_ppomniva_manifest` IS NULL';
    ...
    private function adoptCarriers(bool $dryRun): array
    {
        ...
        if ($mjvpCourierRef) {
            $ppCourierRef = Configuration::get('PPOMNIVA_COURIER_ID_REF');

            if ($ppCourierRef && (int) $ppCourierRef !== (int) $mjvpCourierRef) {
                // ppomniva already has its own carrier — update the legacy carrier to point to ppomniva
                if (!$dryRun) {
                    $db->execute(
                        'UPDATE `' . _DB_PREFIX_ . 'carrier`
                         SET `external_module_name` = \'ppomniva\',
                             `active` = 1
                         WHERE `id_reference` = ' . (int) $mjvpCourierRef . '
                         AND `deleted` = 0
                         ORDER BY `id_carrier` DESC LIMIT 1'
                    );
                    // Store the legacy reference for ppomniva
                    Configuration::updateValue('PPOMNIVA_COURIER_ID_REF', (int) $mjvpCourierRef);
                }
                $log[] = sprintf('%s courier carrier ref %d', $dryRun ? '[dry-run] Would adopt' : 'Adopted', (int) $mjvpCourierRef);
            } elseif (!$ppCourierRef) {
                // No ppomniva carrier yet — adopt the legacy one
                if (!$dryRun) {
                    $db->execute(
                        'UPDATE `' . _DB_PREFIX_ . 'carrier`
                         SET `external_module_name` = \'ppomniva\',
                    ...
                    if ($carrierId) {
                        Configuration::updateValue('PPOMNIVA_COURIER_ID', $carrierId);
                    }
                }
                $log[] = sprintf('%s courier carrier ref %d', $dryRun ? '[dry-run] Would adopt' : 'Adopted', (int) $mjvpCourierRef);
            } else {
        ...
        if ($mjvpPickupRef) {
            $ppPickupRef = Configuration::get('PPOMNIVA_PICKUP_ID_REF');
            ...
            if (!$ppPickupRef) {
                if (!$dryRun) {
                    ...
                    Configuration::updateValue('PPOMNIVA_PICKUP_ID_REF', (int) $mjvpPickupRef);
                    ...
                    if ($carrierId) {
                        Configuration::updateValue('PPOMNIVA_PICKUP_ID', $carrierId);
                    }
                }
    ...
    private function adoptOrderStates(bool $dryRun): array
    {
        ...
        // Reuse the same PS order states if ppomniva hasn't created its own
        $mjvpReady = Configuration::get('MJVP_ORDER_STATE_READY');
        $mjvpError = Configuration::get('MJVP_ORDER_STATE_ERROR');

        if ($mjvpReady && !(int) Configuration::get('PPOMNIVA_STATE_READY')) {
            if (!$dryRun) {
                Configuration::updateValue('PPOMNIVA_STATE_READY', (int) $mjvpReady);
            }
            $log[] = sprintf('%s order state "ready" (ID %d)', $dryRun ? '[dry-run] Would adopt' : 'Adopted', (int) $mjvpReady);
        }

        if ($mjvpError && !(int) Configuration::get('PPOMNIVA_STATE_ERROR')) {
            if (!$dryRun) {
                Configuration::updateValue('PPOMNIVA_STATE_ERROR', (int) $mjvpError);
            }
    ...
    private function countAlreadyMigrated(): int
    {
        ...
        $count = Db::getInstance()->getValue(
            'SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'mjvp_orders` mo
             INNER JOIN `' . _DB_PREFIX_ . 'ppomniva_order` po ON po.`id_order` = mo.`id_order`
             WHERE mo.`id_order` > 0'
        );
