...
/**
 * Persists Omniva-related errors to the dedicated `ppomniva_log` table and
...
class ApiLogger
{
    ...
    /**
     * Log a Omniva API error response. `errors` is the structured list emitted
     * by OmnivaErrorMapper / parseXmlResponse; the first entry's code is
     ...
     * is successfully registered with Omniva so we don't keep large XML
    ...
    public static function clearForOrder(int $idOrder, ?string $orderReference = null): int
    {
        ...
        $deleted = $db->execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'ppomniva_log`
    ...
    public static function cleanup(int $retentionDays): int
    {
        ...
        $db = Db::getInstance();
        $deleted = $db->execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'ppomniva_log` WHERE `date_add` < \'' . pSQL($cutoff) . '\''
        );
    ...
    private static function insert(array $row): void
    {
        ...
        try {
            Db::getInstance()->insert('ppomniva_log', $insert);
        } catch (\Throwable $e) {
    ...
    private static function mirrorToCoreLogger(array $row): void
    {
        ...
        $prefix = $row['source'] !== '' ? $row['source'] . ': ' : 'PPOmniva: ';
