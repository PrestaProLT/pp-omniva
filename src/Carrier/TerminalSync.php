...
namespace PrestaShop\Module\PPOmniva\Carrier;
...
class TerminalSync
{
    ...
    private const TABLE = 'ppomniva_terminal';

    public function __construct(
        private readonly OmnivaApiClient $apiClient,
    ) {
    }
    ...
    public function syncCountry(string $countryCode): int
    {
        ...
        foreach ($terminals as $terminal) {
            ...
            // terminal_id is VARCHAR (Omniva uses alphanumeric ids), so keep it
            // as an escaped string for both the upsert and the prune list.
            $terminalId = pSQL((string) $terminal['id']);
            $seenIds[] = $terminalId;
        ...
        if (!empty($seenIds)) {
            ...
            $db->execute(
                ...
                . ' AND `terminal_id` NOT IN (' . $quotedIds . ')'
            );
        }
    ...
    /**
     * Get a single terminal by its Omniva ID.
     */
