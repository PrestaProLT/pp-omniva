...
class OmnivaShippingCalculator
{
    /**
     * Check if Omniva carriers should be disabled for this cart.
     * Returns true if carriers should be HIDDEN.
     *
     * Checks: if PPOMNIVA_DISABLE_PASSPHRASE is set, queries each product's
    ...
    {
        $passphrase = Configuration::get('PPOMNIVA_DISABLE_PASSPHRASE');
    ...
    public function hasAvailableTerminals(int $idCart): bool
    {
        ...
        $terminalCount = (int) $db->getValue(
            'SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ppomniva_terminal`
